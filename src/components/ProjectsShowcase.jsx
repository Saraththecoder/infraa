...
export default function ProjectsShowcase() {
  ...
  const getAspectClass = (index) => {
    const mod = index % 4;
    if (mod === 0) return "aspect-[4/5] h-[600px] md:h-auto";
    if (mod === 1) return "aspect-[3/2] h-[360px] md:h-auto";
    if (mod === 2) return "aspect-[3/2] h-[360px] md:h-auto";
    return "aspect-[4/5] h-[600px] md:h-auto";
  };
    ...
    <section id="projects" className="section-pad bg-white relative overflow-hidden min-h-screen">
      <div className="container-custom relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-ak-border pb-4 md:pb-2 self-start md:self-end">
            {categories.map((tab) => (
              <button
                key={tab.id}
        ...
        {/* Alternating Masonry-Style CSS Grid (2 Columns) */}
        <motion.div 
          layout
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, idx) => (
              <motion.div
                ...
                key={project.name}
                className={`group relative overflow-hidden rounded-[4px] shadow-ak-sm hover:shadow-ak-lg cursor-pointer bg-ak-offwhite ${getAspectClass(idx)}`}
              >
                {/* Image */}
                <img
                  src={project.image}
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-8 text-left">
                  ...
                  {/* Status Pill */}
                  <div className="w-fit bg-ak-gold text-white text-[10px] font-bold tracking-[0.15em] uppercase px-3 py-1 rounded-[2px] mb-4">
                  ...
                  <div className="overflow-hidden max-h-0 group-hover:max-h-20 transition-all duration-500 ease-in-out">
                    <p className="text-white/85 font-sans text-sm mt-4 leading-relaxed pr-6">
                      {project.description}
                    </p>
                  </div>

                  {/* Arrow Icon in bottom-right */}
                  <div className="absolute bottom-8 right-8 text-white w-8 h-8 rounded-full border border-white/20 flex items-center justify-center group-hover:border-ak-gold group-hover:text-ak-gold transition-colors duration-300">
